...
class Admin_Page {
	...
	public function __construct( array $args ) {
		...
		$meta_boxes = new \UwooE\RegisterMetaBoxes\Register_Meta_Boxes( $this->slug );
	}
	...
	public function activate_default_tab() {
		...
		if ( ! isset( $_GET['page'] ) || sanitize_text_field(wp_unslash($_GET['page'])) !== $this->slug ) {
			return;
		}
	...
	public function add_page(){
		...
		if ( is_array( $this->help_section ) && isset( $this->help_section['tabs'] ) ) {
			...
			new \UwooE\HelpTabs\Help_Tabs( $this->page, $this->help_section['tabs'], $sidebar );
		}
	}
	...
	public function render_page(){ ?>
		 ...
		 <div class="wrap">
		 	...
		 	<?php do_action( "uwooe_settings_page_top" ); ?>
			...
			<h1>
				...
				<?php do_action( 'uwooe_settings_page_title_action' ); ?>
			</h1>

			<?php do_action( "uwooe_settings_page_before_settings" ); ?>
			...
			<form name="<?php echo $this->slug; ?>_admin_form" id="<?php echo $this->slug; ?>_admin_form" action="?page=<?php echo $this->slug; ?>&save-settings=true" method="post">
				...
				<div id="poststuff">
					 ...
					 <div id="post-body" class="metabox-holder columns-<?php echo 1 == get_current_screen()->get_columns() ? '1' : '2'; ?>"> 
						...
						<?php do_action( "uwooe_settings_page_body_end" ); ?>	     					
			...
			<div id="confirmation_alert" style="display:none;">
				<div class="popup-content">
					...
					<div class="input-field">
						<input type="button" class="btn" name="btn_confirm" id="btn_confirm" onclick="confirm_submission()" value="<?php echo __('Confirm', 'ultimo-wooemail');?>"/>
					</div>
				</div>
			</div>
			<div id="locked_alert" style="display:none;">
				<div class="popup-content locked">
					...
					<div class="input-field">
						<a class="btn" href="https://ultimoepro.com/item/ultimo-woocommerce-email-master/" target="_blank"><?php echo __('Upgrade Now', 'ultimo-wooemail');?></a>
						<a class="btn btn-cancel" href="javascript:void(0)" onclick="close_poup();"><?php echo __('Not Now', 'ultimo-wooemail');?></a>
					</div>
				</div>
			</div>
			<div class="support-section">
				<button type="button" onclick="support_popup()" class="btn btn-support" style="display:block;" align="center"><?php echo __('Contact Support', 'ultimo-wooemail');?></button>
				<div id="contactsupport" style="display:none;">
					<div class="popup-content support-form">
						...
						<div id="mc_embed_signup" class="support-form">
							<form action="https://ultimocmsbooster.us20.list-manage.com/subscribe/post?u=7243c7eda31cdc78043732822&amp;id=c751881d59" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate>
								<div id="mc_embed_signup_scroll">
									<h2><?php echo __('Get In Touch with Us!', 'ultimo-wooemail');?></h2>
									<div class="mc-field-group">
										<label for="mce-EMAIL"><?php echo __('Email Address', 'ultimo-wooemail');?> </label>
										<input type="email" value="" name="EMAIL" class="required email" id="mce-EMAIL">
									</div>
									<div class="mc-field-group">
										<label for="mce-MESSAGE"><?php echo __('Message', 'ultimo-wooemail');?> </label>
										<textarea rows="5" name="MESSAGE" class="" id="mce-MESSAGE"></textarea>
									</div>
									<div id="mce-responses" class="clear">
										<div class="response" id="mce-error-response" style="display:none"></div>
										<div class="response" id="mce-success-response" style="display:none"></div>
									</div>    <!-- real people should not fill this in and expect good things - do not remove this or risk form bot signups-->
									<div style="position: absolute; left: -5000px;" aria-hidden="true"><input type="text" name="b_7243c7eda31cdc78043732822_c751881d59" tabindex="-1" value=""></div>
									<div class="clear"><input type="submit" value="<?php echo __('Send', 'ultimo-wooemail');?>" name="subscribe" id="mc-embedded-subscribe" class="button"></div>
								</div>
							</form>
						</div>
						<script type='text/javascript' src='//s3.amazonaws.com/downloads.mailchimp.com/js/mc-validate.js'></script><script type='text/javascript'>(function($) {window.fnames = new Array(); window.ftypes = new Array();fnames[0]='EMAIL';ftypes[0]='email';fnames[2]='MESSAGE';ftypes[2]='text';}(jQuery));var $mcj = jQuery.noConflict(true);</script>
						<!--End mc_embed_signup-->
					</div>
				</div>
			</div>
			...
			<script type="text/javascript">
				...
				jQuery("input.switch").click(function(){
					...
					else
					{
						...
						{
							var message = "<?php echo __( 'Click confirm to activate this tool.', 'ultimo-wooemail' );?>";
							jQuery(this).addClass("unchecked");
						}
						else
						{
							var message = "<?php echo __( 'Click confirm to deactivate this tool.', 'ultimo-wooemail' );?>";
							jQuery(this).addClass("checked");
						}
						jQuery("#alert_message").html(message);
						tb_show('', '#TB_inline?height=300&width=auto&unvq=true&inlineId=confirmation_alert');
					}
				...
				{
					jQuery("#uwooe_admin_form").submit();
				}
				function close_poup()
				{
			...
			<?php do_action( "uwooe_settings_page_after_settings" ); ?>
	...
	public function render_tabs( $current = '' ) {
		...
		if ( ! empty( $this->tabs ) ) : ?>
			...
			<?php do_action( "uwooe_settings_page_before_tabs" ); ?>
			...
			<h2 class="nav-tab-wrapper">
				...
				<?php foreach ( $this->tabs as $key => $tab ) : ?>
					...
					<a href="<?php echo add_query_arg( 'tab', $key, UWOOE_SETTINGS_PAGE_URL ); ?>" class="nav-tab<?php echo $active; ?>"><?php echo $tab; ?></a>

				<?php endforeach; ?>

			</h2>

			<?php do_action( "uwooe_settings_page_after_tabs" ); ?>
