	...
	if ( ! class_exists( 'woocommerce_aweber' ) ) {
		...
		class woocommerce_aweber
		{
			...
			function settings_menu( $settings_tab ) {
				$settings_tab['woo_aweber'] = __( 'Aweber', 'ultimo-wooemail' );
			...
			function auth_redirect() {
				if ( isset( $_GET[ 'tab' ] ) && sanitize_text_field(wp_unslash($_GET[ 'tab' ])) == 'woo_aweber' && isset( $_GET[ 'awauth' ] ) && ! isset( $_GET[ 'saved' ] ) ) {
					$admin_options = get_option( $this->adminOptionsName );
					$pageURL = admin_url( 'admin.php?page=wc-settings&tab=woo_aweber' );

					if ( sanitize_text_field(wp_unslash($_GET[ 'awauth' ])) == 'false' ) {
						$admin_options[ 'access_token' ] = null;
						$admin_options[ 'access_secret' ] = null;
						update_option( $this->adminOptionsName, $admin_options );
					} elseif ( sanitize_text_field(wp_unslash($_GET[ 'awauth' ])) == 'true' ) {
						$aweber = $this->_get_aweber_api();
			...
			function settings_update() {
				...
				if ( isset( $_POST[ 'wc_aw_subscribe_label' ] ) )
					$admin_options[ 'subscribe_label' ] = sanitize_text_field($_POST[ 'wc_aw_subscribe_label' ]);
				...
				if ( isset( $_POST[ 'wc_aw_subscribe_id' ] ) )
					$admin_options[ 'subscribe_id' ] = sanitize_text_field($_POST[ 'wc_aw_subscribe_id' ]);
				update_option( $this->adminOptionsName, $admin_options );
			} // End settings_update()
			...
			function add_dashboard_widgets() {
				wp_add_dashboard_widget( 'woocommerce_aweber_subscriber_dashboard', __( 'AWeber Newsletter Subscribers', 'ultimo-wooemail' ), array( $this, 'subscriber_stats' ) );
				...
				if ( $admin_options[ 'access_token' ] ) {
					if ( ! $html = get_transient( 'woo_aweber_stats', 60 * 60 ) ) {
						try {
							...
							$html .= '<li><strong>' . $list->total_subscribed_subscribers . '</strong> ' . __( 'Total subscribers', 'ultimo-wooemail' ) . '</li>';
							$html .= '<li><strong>' . $list->total_unsubscribed_subscribers . '</strong> ' . __( 'Unsubscribes', 'ultimo-wooemail' ) . '</li>';
							$html .= '<li><strong>' . $list->total_subscribers_subscribed_today . '</strong> ' . __( 'Subscribed today', 'ultimo-wooemail' ) . '</li>';
							$html .= '<li><strong>' . $list->total_subscribers_subscribed_yesterday . '</strong> ' . __( 'Subscribed yesterday', 'ultimo-wooemail' ) . '</li>';
							$html .= '<li><strong>' . $list->total_unconfirmed_subscribers . '</strong> ' . __( 'Unconfirmed subscribers', 'ultimo-wooemail' ) . '</li>';
							$html .= '</ul>';
							set_transient( 'woo_aweber_stats', $html, 60 * 60 );
						} catch ( Exception $e ) {
							$admin_options[ 'access_token' ] = null;
							delete_transient( 'woo_aweber_stats' );
							echo '<div class="error inline"><p>' . __( 'Please authorise WooCommerce to access your AWeber account.', 'ultimo-wooemail' ) . '</p></div>';
						}
					}
					echo $html;
				} else {
